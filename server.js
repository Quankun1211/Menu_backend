...
import {
  recoverGatewayCompletedRefunds,
  retryFailedRefunds,
} from "./controller/orderCancellationController.js"
import {
  errorHandler,
  notFoundHandler,
  requestLogger,
...
app.use((req, res, next) => {
  ...
  console.log(
    "Authorization:",
    req.headers.authorization ? "Có" : "Không"
  )
...
io.on("connection", (socket) => {
  ...
  if (socket.user.role === "shipper") {
    socket.join(`shipper:${socket.user._id}`)
  }
  ...
  socket.on("join_order", async (orderId) => {
    try {
      ...
      console.error("[SOCKET JOIN ORDER ERROR]", error)
    }
  })
...
app.get("/health", async (_req, res) => {
  try {
    const mongoReady =
      (await import("mongoose")).default.connection.readyState === 1
  ...
  try {
    ...
    const runPaymentExpiry = async () => {
      ...
      try {
        ...
        if (
          locked &&
          (await redisClient.get("lock:payment-expiry")) ===
            lockToken
        ) {
